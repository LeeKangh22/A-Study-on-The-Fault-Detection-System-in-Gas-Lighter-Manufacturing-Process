...
UPPER_NORMAL_BOUND_RATIO = 18 / 39
LOWER_NORMAL_BOUND_RATIO = 27 / 39
...
class Yolo:
    ...
    def initialize_camera(self):
        ...
        self.camera.create_gstreamer_pipeline(
            ...
            framerate       = 30,       # Camera framerate      (10FPS used for this application) 
    ...
    def set_screen_auto(self):
        ...
        self.upper_normal_line = self.upper_std_line + int(self.lighter_height * (UPPER_NORMAL_BOUND_RATIO))  # Upper boundary of normal area.
        self.lower_normal_line = self.upper_std_line + int(self.lighter_height * (LOWER_NORMAL_BOUND_RATIO))  # Lower boundary of normal area.
    ...
    def set_screen_manual(self):
        ...
        self.upper_normal_line = self.std_upper + int(self.lighter_height * (UPPER_NORMAL_BOUND_RATIO))  # Upper boundary of normal area.
        self.lower_normal_line = self.std_upper + int(self.lighter_height * (LOWER_NORMAL_BOUND_RATIO))  # Lower boundary of normal area.
        img = self.get_camera_image()
        self.test(cv2.cvtColor(img, cv2.COLOR_GRAY2RGB), flag = True)
        self.save_lighter_info()
        # Unnecessary printing for debugging purpose
...
class LighterApp(QWidget, SystemInfo, Yolo):
    ...
    def show_result(self):
        ...
        for num, value in enumerate(self.lighter_error):
            if value == 1: result += (str(num + 1) + "번 미달 / ")
            if value == 2: result += (str(num + 1) + "번 초과 / ")
